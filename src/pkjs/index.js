...
function mergeConfiguration(base, overlay) {
  var merged = cloneConfiguration(base);
  var key;
  for (key in overlay) {
    if (
      overlay.hasOwnProperty(key) &&
...
function loadConfigurationFromStorage() {
  var stored = window.localStorage.getItem("configuration");
  if (!stored) {
  ...
  try {
    return mergeConfiguration(
      ...
      parseConfigurationResponse(stored)
    );
  } catch (parseError) {
    console.log("error parsing config from localStorage: " + parseError);
    return cloneConfiguration(DEFAULT_CONFIGURATION);
  }
}

var configuration = cloneConfiguration(DEFAULT_CONFIGURATION);
...
function fetchUvIndex(lat, lon, callback) {
  var url =
    "https://currentuvindex.com/api/v1/uvi?latitude=" +
...
function locationSuccess(pos, weatherConfiguration) {
  ...
  SendToPebble(pos, 0, weatherConfiguration);
}

function locationError(err, weatherConfiguration) {
  ...
  if (window.localStorage.getItem("last_location")) {
    ...
    last_location = JSON.parse(window.localStorage.last_location);
    //var test = JSON.parse(window.localStorage.configuration);
    console.log(
      "loaded last location = " + JSON.stringify(last_location, null, 2)
    );
  ...
  if (last_detected_loc.lat === 0.0 && last_detected_loc.lon === 0.0) {
    ...
    SendToPebble(pos, 1, weatherConfiguration);
  } else {
    pos = {
      coords: {
        latitude: last_detected_loc.lat,
        longitude: last_detected_loc.lon
      }
    };
    SendToPebble(pos, 0, weatherConfiguration);
  }
...
function SendToPebble(pos, use_default, weatherConfiguration) {
  var conf = cloneConfiguration(weatherConfiguration || configuration);
  var url;
  var url_forecast;
  ...
  // Construct URL
  console.log("conf.auto_loc = " + conf.autodetect_loc);
  //conf.OWM_API_KEY = OWM_DEFAULT_API_KEY; //TODO
  if (
    typeof conf.OWM_API_KEY === "string" ||
    conf.OWM_API_KEY instanceof String
  ) {
    if (conf.OWM_API_KEY == "default") {
      conf.OWM_API_KEY = OWM_DEFAULT_API_KEY;
    }
    if (String(conf.OWM_API_KEY).length < 20) {
      conf.OWM_API_KEY = OWM_DEFAULT_API_KEY;
    }
  } else {
    conf.OWM_API_KEY = OWM_DEFAULT_API_KEY;
  }
  console.log("Using OWM API KEY: " + conf.OWM_API_KEY);
  if (use_default === 0 && conf.autodetect_loc) {
    url =
    ...
    console.log("conf.def_loc = " + conf.default_loc);
    var city_name_req = conf.default_loc;
    // Construct URL
    url =
      "http://api.openweathermap.org/data/2.5/weather?APPID=" +
      conf.OWM_API_KEY +
  ...
  xhrRequest(url_forecast, "GET", function (responseText) {
    ...
    xhrRequest(url, "GET", function (responseText) {
      ...
      if (!WeatherDataJSON_error) {
        ...
        if (conf.speed_unit === 0) {
          speed_unit_conversion_factor = 3.6; //m/s -> km/h
        } else if (conf.speed_unit == 1) {
          speed_unit_conversion_factor = 2.236; //m/s -> mph
        }
        var wind_speed =
          WeatherDataJSON.wind.speed * speed_unit_conversion_factor;
        if (wind_speed < 10) {
        ...
        if (conf.speed_unit === 0) wind_speed_unit = "km/h";
        if (conf.speed_unit == 1) wind_speed_unit = "mph";
        ...
        var warn_location = 0;
        if (conf.autodetect_loc && use_default) {
          //tried autodection of location, but could not get the lat long values from phone, so used default location set by the user.
          warn_location = 2;
          console.log(
            "Tried autodection of location, but could not get the lat long values from phone."
          );
        }
        if (conf.autodetect_loc === 0 && use_default === 0) {
          //detected location, but used user input
          console.log("Could autodect location, but used user input instead.");
          console.log(
            " |lat_autodetect - user_lat| = " +
              Math.abs(WeatherDataJSON.coord.lat - pos_lat)
          );
          console.log(
            " |lon_autodetect - user_lon| = " +
              Math.abs(WeatherDataJSON.coord.lon - pos_lon)
          );
          console.log("pos_lat                   = " + pos_lat);
          console.log(
            "WeatherDataJSON.coord.lat = " + WeatherDataJSON.coord.lat
          );
          console.log("pos_lon                   = " + pos_lon);
          console.log(
            "WeatherDataJSON.coord.lon = " + WeatherDataJSON.coord.lon
          );
          if (
            Math.abs(WeatherDataJSON.coord.lat - pos_lat) > 0.3 &&
            Math.abs(WeatherDataJSON.coord.lon - pos_lon) > 0.5
          ) {
            console.log(
              "User input location differs from autodeteced location."
            );
            warn_location = 1;
          }
        }
        ...
        if (!ForecastDataJSON_error) {
          console.log(
            "forecast_list has " + ForecastDataJSON.cnt + " elements"
          );
          var i;
          for (i = 0; i < Math.min(ForecastDataJSON.cnt, 8); i++) {
            // 8 entries means 24 hours for 3 hour forecast
            console.log(
              "forecast_list[" +
                i +
                "].dt_text = " +
                ForecastDataJSON.list[i].dt_txt +
                "; T = " +
                (ForecastDataJSON.list[i].main.temp - 273.15) +
                " C"
            );
            Forecast.TempMin = Math.min(
              ForecastDataJSON.list[i].main.temp,
              Forecast.TempMin
            );
            Forecast.TempMax = Math.max(
              ForecastDataJSON.list[i].main.temp,
              Forecast.TempMax
            );
          }
        ...
        function sendWeatherInfo(uvIndex) {
          ...
          switch (conf.weatherLine1) {
            ...
            case 5:
              if (Forecast.TempMin == 10000 || Forecast.TempMax === 0) {
                ...
                if (conf.degree_f) {
                  weather_Line_1 =
                    Math.round((Forecast.TempMax - 273.15) * 1.8 + 32) +
                    "°/" +
                    Math.round((Forecast.TempMin - 273.15) * 1.8 + 32) +
                    "°F";
                } else {
                  weather_Line_1 =
                    Math.round(Forecast.TempMax - 273.15) +
                    "°/" +
                    Math.round(Forecast.TempMin - 273.15) +
                    "°C";
          ...
          console.log(
            "weather_Line_1 = " +
              weather_Line_1.replace("°", " ").replace("°", " ")
          );

          switch (conf.weatherLine2) {
            case 1:
          ...
          console.log(
            "weather_Line_2 = " +
              weather_Line_2.replace("°", " ").replace("°", " ")
          );

          switch (conf.weatherLine3) {
            case 1:
          ...
          console.log(
            "weather_Line_3 = " +
              weather_Line_3.replace("°", " ").replace("°", " ")
          );

          switch (conf.weatherLine4) {
            case 1:
          ...
          console.log(
            "weather_Line_4 = " +
              weather_Line_4.replace("°", " ").replace("°", " ")
          );
          ...
          if (conf.weatherLine3 === 0) weather_string_2 = weather_Line_4;
          if (conf.weatherLine4 === 0) weather_string_2 = weather_Line_3;
          console.log(
            "weather_string_2 is: \n" +
          ...
          if (CLOUDPEBBLE) {
            weather_string_1 = weather_string_1
              .replace("°", "__")
              .replace("°", "__");
            weather_string_2 = weather_string_2
              .replace("°", "__")
              .replace("°", "__");
          }

          if (conf.autodetect_loc == 2) warn_location = 0;
        ...
        if (usesUvIndex(conf)) {
          fetchUvIndex(
            WeatherDataJSON.coord.lat,
            WeatherDataJSON.coord.lon,
            function (fetchedUvIndex) {
...
function getWeather() {
  var weatherConfiguration = loadConfigurationFromStorage();
  ...
  if (weatherConfiguration.autodetect_loc == 2) {
    var pos = {
      coords: { latitude: 0, longitude: 0 }
    };
    SendToPebble(pos, 1, weatherConfiguration);
    ...
    navigator.geolocation.getCurrentPosition(
      //could also use navigator.geolocation.watchPosition() ?
      function (pos) {
        locationSuccess(pos, weatherConfiguration);
      },
      function (err) {
        locationError(err, weatherConfiguration);
      },
      options
    );
  }
...
var COLOR_CONFIG_KEYS = {
  ...
  bottombar_bg: true,
  bottombar_txt: true,
  seperation_lines: true,
  weather_icon_colors: true,
  battery_icon_colors: true
};
...
function parseConfigurationResponse(response) {
  ...
  // Pebble may pass an already-decoded fragment; decoding again breaks on % in date_format.
  var decoded =
  ...
  return JSON.parse(decoded.substring(0, end + 1));
}

function compactConfigurationForStorage(config) {
  if (!isApliteWatch()) {
    return config;
  }
  var compact = {};
  var key;
  for (key in config) {
    if (config.hasOwnProperty(key) && !COLOR_CONFIG_KEYS[key]) {
      // If aplite, we filter out any of the color settings to prevent memory issues
...
function buildCoreSettingsDictionary(configuration) {
  ...
  var dict = {
    ...
    "KEY_SET_RIGHT_SIDE": sanitizeSettingInt(configuration.right_side),
    "KEY_WEATHER_UPDATE_INT": sanitizeSettingInt(
      configuration.weatherUpdateInt
    ),
...
function buildColorSettingsDictionary(configuration) {
  var dict = {
    ...
    "KEY_SET_CLOCKALTTXT_COLOR": sanitizeSettingInt(
      configuration.clock_alt_txt
    ),
    "KEY_SET_BOTTOMBARBG_COLOR": sanitizeSettingInt(configuration.bottombar_bg),
    "KEY_SET_BOTTOMBARTXT_COLOR": sanitizeSettingInt(
      configuration.bottombar_txt
    ),
    "KEY_SET_SEPERATIONLINES_COLOR": sanitizeSettingInt(
      configuration.seperation_lines
    ),
    "KEY_SET_WEATHERICON_COLOR": sanitizeSettingInt(
      configuration.weather_icon_colors
    ),
    "KEY_SET_BATTERYICON_COLOR": sanitizeSettingInt(
      configuration.battery_icon_colors
    )
  };
...
function mergeMessageDictionaries(base, overlay) {
  var merged = {};
  var key;
  for (key in base) {
...
function buildSettingsDictionary(configuration) {
  var coreSettings = buildCoreSettingsDictionary(configuration);
  if (isApliteWatch()) {
    return coreSettings;
  }
  return mergeMessageDictionaries(
    coreSettings,
    buildColorSettingsDictionary(configuration)
  );
}

function sendSettingsToWatch(configuration, onSuccess, onFailure) {
  var settings = buildSettingsDictionary(configuration);
...
Pebble.addEventListener("ready", function (e) {
  ...
  configuration = loadConfigurationFromStorage();
});
...
Pebble.addEventListener("showConfiguration", function (e) {
  ...
  Pebble.openURL(
    "https://dniku.github.io/pebble-mss/pebble_m7s_config_v15.5.html"
  );
  //Pebble.openURL("http://192.168.1.2:13371?v=2/");
...
Pebble.addEventListener("webviewclosed", function (e) {
  ...
  try {
    configuration = mergeConfiguration(
      DEFAULT_CONFIGURATION,
      parseConfigurationResponse(e.response)
    );
  } catch (parseError) {
    console.log("Configuration parse failed: " + parseError);
    return;
  }
  var configuration_str = JSON.stringify(configuration);
  console.log(
    "Configuration window returned: " + JSON.stringify(configuration, null, 2)
  );

  if (
    configuration_str.charAt(0) == "{" &&
    configuration_str.slice(-1) == "}" &&
    configuration_str.length > 5
  ) {
    window.localStorage.configuration = JSON.stringify(
      compactConfigurationForStorage(configuration)
    );

    if (
      typeof configuration.OWM_API_KEY === "string" ||
      configuration.OWM_API_KEY instanceof String
    ) {
    ...
    //Weather refresh is triggered by the watch (doUpdateWeather -> appmessage).
    sendSettingsToWatch(
      configuration,
      function (e) {
        console.log("Settings data transfered successfully.");
